...
def comprehend(mystring):
    ...
    for i in range(len(data_list)):
        try:
            ...
            if i<len(data_list)-1 and i>0 and data_list[i+1]==data_list[i-1]:
...
data=st.text_input("Enter all the data values in increasing order, separated by commas. For suppressed entries, enter how many there are in each group between two underscores: \" \_ { number of suppressed entries in the group } \_ \" . Note that the values within each group of suppressed entries are assumed to lie strictly between the two values that the group is sandwiched between, unless those two values are identical, in which case the suppressed entries are assumed to be equal to them.")
data_copy=data
...
if marker==True:
    ...
    st.markdown(f"The smallest value having a percentile rank of at least ${pr}\\%$ is ${try_int(smallest_value)}$. This value has percentile rank equal to ${pr_smallest_value}\\%$.")
